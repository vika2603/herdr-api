...
This document is the architecture contract for `github.com/vika2603/herdr-api`
and the interface agreement between the parts that are developed in parallel.
Every statement about the protocol is backed by `schema/herdr-api.schema.json`
(herdr 0.9.0, protocol 22) or by the herdr v0.9.0 sources.

## Goal

Provide a Go client for the Herdr socket API and runtime helpers for writing
Herdr plugins in Go. Types and method wrappers are generated from the official
...
| `cmd/herdr-apigen/` `internal/gen/` | Generator | `feat/gen` |
...
| `results_gen.go` | `Result` interface, the 64 result structs, `DecodeResult` |
...
- `ResponseResult` variants: `<Pascal(type)>Result`, for example
  `pane_info→PaneInfoResult`, `ok→OKResult`, `pong→PongResult`.
...
```go
type Result interface{ ResultType() string }          // the "type" constant
...
- To upgrade herdr: replace `schema/herdr-api.schema.json`, complete
