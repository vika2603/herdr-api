...
func emitEventsFile(pkg *Package) string {
	...
	c.blank()
	c.doc("NextEvent reads the next line the server pushes and decodes its payload. The errors of Next are returned unchanged.")
	c.line("func (s *Stream) NextEvent(ctx context.Context) (Event, error) {")
	c.line("\traw, err := s.Next(ctx)")
	c.line("\tif err != nil {")
	c.line("\t\treturn nil, err")
	c.line("\t}")
	c.line("\treturn DecodeEvent(raw.Event, raw.Data)")
	c.line("}")
...
func emitMethod(c *code, m *Method) {
	...
	case MethodMulti:
		...
		c.line("\t\treturn nil, err")
		c.line("\t}")
		c.line("\treturn DecodeResult(raw)")
		c.line("}")
	default:
		...
		c.line("\t\treturn nil, err")
		c.line("\t}")
		c.line("\tresult, err := DecodeResult(raw)")
		c.line("\tif err != nil {")
		c.line("\t\treturn nil, err")
		c.line("\t}")
		c.printf("\ttyped, ok := result.(*%s)\n", m.ResultType)
		c.line("\tif !ok {")
		c.printf("\t\treturn nil, &UnexpectedResultError{Method: %s, Want: %q, Got: result.ResultType()}\n", m.Const, m.ResultTag)
		c.line("\t}")
